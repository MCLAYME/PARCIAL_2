...
void ADonkeyKongCharacter::DispararFuego()
{
	FVector SpawnLocation = GetActorLocation() + FVector(100, 0, 0); // Ajusta la posición de aparición
	FRotator SpawnRotation = GetActorRotation();
	GetWorld()->SpawnActor<AFuegoProjectile>(AFuegoProjectile::StaticClass(), SpawnLocation, SpawnRotation);
}

void ADonkeyKongCharacter::DispararHielo()
{
	FVector SpawnLocation = GetActorLocation() + FVector(100, 0, 0); // Ajusta la posición de aparición
	FRotator SpawnRotation = GetActorRotation();
...
void ADonkeyKongCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("DispararFuego", IE_Pressed, this, &ADonkeyKongCharacter::DispararFuego);
	PlayerInputComponent->BindAction("DispararHielo", IE_Pressed, this, &ADonkeyKongCharacter::DispararHielo);
